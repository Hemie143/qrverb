...
class TestScanner(TestCase):
    ...
    def test_broken_canvas_sizes(self):
        self.assertRaisesMsg(QrImageRecognitionException, self._get_res_scanner(EXAMPLES.broken_pattern_1).parseImage,
                             'Top-left position pattern not aligned with the top-right one')
        self.assertRaisesMsg(QrImageRecognitionException, self._get_res_scanner(EXAMPLES.broken_pattern_2).parseImage,
                             'Top-left position pattern not aligned with the top-right one')
        self.assertRaisesMsg(QrImageRecognitionException, self._get_res_scanner(EXAMPLES.broken_pattern_3).parseImage,
                             'Top-left position pattern not aligned with the bottom-left one')
        self.assertRaisesMsg(QrImageRecognitionException, self._get_res_scanner(EXAMPLES.broken_too_light).parseImage,
